...
func makeProtoStringerFieldCoder(ft reflect.Type) pointerCoderFuncs {
	return pointerCoderFuncs{
		...
		unmarshal: func(b []byte, p pointer, wtyp protowire.Type, f *coderFieldInfo, opts unmarshalOptions) (out unmarshalOutput, err error) {
			...
			if err := ps.Interface().(pref.ProtoStringer).ParseProtoString(d); err != nil {
				return out, err
			}
			return out, nil
		},
		merge: func(dst, src pointer, _ *coderFieldInfo, _ mergeOptions) {
			...
			if srcp.IsNil() {
				...
				v, err := srcp.Interface().(pref.ProtoStringer).ProtoString()
				if err != nil {
					panic(err)
				}
				if err := dstp.Interface().(pref.ProtoStringer).ParseProtoString(v); err != nil {
					panic(err)
				}
			}
		},
...
func makeProtoStringerSliceFieldCoder(ft reflect.Type) pointerCoderFuncs {
	return pointerCoderFuncs{
		...
		unmarshal: func(b []byte, p pointer, wtyp protowire.Type, f *coderFieldInfo, opts unmarshalOptions) (out unmarshalOutput, err error) {
			...
			if err := ps.Interface().(pref.ProtoStringer).ParseProtoString(v); err != nil {
				return out, err
			}
		...
		merge: func(dst, src pointer, f *coderFieldInfo, opts mergeOptions) {
			for _, sp := range src.PointerSlice() {
				ps := reflect.New(ft.Elem())
				v, err := sp.AsValueOf(ft.Elem()).Interface().(pref.ProtoStringer).ProtoString()
				if err != nil {
					panic(err)
				}
				if err := ps.Interface().(pref.ProtoStringer).ParseProtoString(v); err != nil {
