...
func newSingularConverter(t reflect.Type, fd pref.FieldDescriptor) Converter {
	...
	case pref.StringKind:
		...
		if t.Kind() == reflect.Ptr && t.Implements(reflect.TypeOf((*pref.ProtoStringer)(nil)).Elem()) {
...
func (c *protoStringerConverter) PBValueOf(v reflect.Value) pref.Value {
	if v.Type() != c.goType {
		panic(fmt.Sprintf("invalid type: got %v, want %v", v.Type(), c.goType))
	}
	s, err := v.Interface().(pref.ProtoStringer).ProtoString()
	if err != nil {
		panic(err)
	}
	return pref.ValueOfString(s)
}
func (c *protoStringerConverter) GoValueOf(v pref.Value) reflect.Value {
	goVal := reflect.New(c.goType.Elem()).Interface().(pref.ProtoStringer)
...
func (c *protoStringerConverter) IsValidPB(v pref.Value) bool {
	_, ok := v.Interface().(string)
	return ok
}
...
func (c *protoStringerConverter) New() pref.Value {
	return c.PBValueOf(reflect.New(c.goType.Elem()))
}
func (c *protoStringerConverter) Zero() pref.Value {
